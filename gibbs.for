...
c     change '#IJ' to '<(maxmol-1)*maxmol/2>' nol
c     change '#MO' to '<maxmol>' nol
c     change '#NA' to '<maxslv*maxmol>' nol
c     change '#RG' to '<maximumn number of rdf grids>' nol
c     change '#SV' to '<maxslv>' nol
c     change '#CV' to '<maximum number of cavities>' nol
c     change '#GX'  to '<maximum number of cavity grids in 1 dimension>' nol
      character*80 label
     ...
     -  cut2g,cut2r,vming,vmaxg,vminr,vmaxr
      ...
      common /genslv/ c6(#SV),c12(#SV),cc6(#SV,#SV),cc12(#SV,#SV),qqg(#SV,#SV),
     -                siglj(#SV),epslj(#SV)
     ...
     -  nvacc,nvaccg,nvaccr,nmccnt,ncnt
      ...
      common /grid/ ncover(#GX,#GX,#GX),igfree(#CV),ngfree,ngrid,
     -  ng1,ng2,ng3,fltngr,fltng3,grid,e2g,e2gm,rsppe2,rspme2
      common /coord/ relcrd(3,#SV),cslv(3,#SV),qslv(#SV),islv(#SV)
      common /dploy/ rsi(3,#SV),crmi(3),orj(3,3),cedgs,rotax,delvol
      common /ctotal/ cg(3,#NA,2),cr(3,#NA,2),org(3,3,#MO),
     -  orr(3,3,#MO),crmg(3,#MO),crmr(3,#MO)
      real*8 besg,besr,etotog,etotng,etotor,etotnr
      common /energy/ besg(#MO,2),texg(#MO,2),texog(#MO,2),
     -                exg(#IJ,2),
     -                besr(#MO,2),texr(#MO,2),texor(#MO,2),
     -                exr(#IJ,2),
     -                etotog,etotng,etotor,etotnr
      real*8 angcrg,angcrr
      common /distr/ angcrg(#RG),angcrr(#RG),nrg(#RG),nrr(#RG),
     -  rdbg(10,#RG),rdbr(10,#RG),angcbg(10,#RG),angcbr(10,#RG),
     ...
     -  lenblk,nblk
      ...
      common /cavprb/ ndistg(#MO),ndistr(#MO),pming,pmaxg,pminr,pmaxr
      common /cntrl/ ceg(100),cer(100),cvg(100),cng(100)
      common /seed/ ix
      common /texz/ tex0(#MO)
      character*8 itg,itr
      common /label/ itg,itr
      dimension rno(4),iopn(40),islv1(#SV)
      real*8 ediff
      character*80 line
...
1041  format(1h1,//' Gibbs-ensemble Monte Carlo program',
     ...
     -  ' Maximum number of molecules=#MO',/,
     -  ' Maximum number of atoms/molecule=#SV',/,
     -  ' Maximum number of cavity grids=#GX**3',/,
     -  ' Maximum number of cavities=#CV',/)
...
c       Molecular LJ correction
        corlng=-elr(6,ct,1.0,nslv,nslv,relcrd,relcrd,cc6,#SV) +
     -         elr(12,ct,1.0,nslv,nslv,relcrd,relcrd,cc12,#SV) 
        qsum=0.0
        do 10 i=1,nslv
...
1000  format(' ***** ERROR: number of cavities in the initial ',
     -  'configuration exceeds capacity (#CV)',/,
...
c*****Perform a molecule displacement try
      dimension c(3,#NA),crm(3,#MO),orient(3,3,#MO)
      real*8 tesi,etoto,etotn,usum,uusum
      dimension tesi(#MO),tex(#MO),texo(#MO),ex(#IJ)
      character*80 label
      common /id/ label(2),temp,fkt,cutof,cutof2,edgea,edge,hlfedg,
     -  prsag,prsar,pressg,pressr,bparg,bparr
      common /sreal/ eming,eminr,emaxg,emaxr,vol,volg,volr,vfacg,vfacr,
...
c=====perform an insertion or deletion attempt based on grid search
      ...
      dimension c(3,#NA),crm(3,#MO),orient(3,3,#MO),tex(#MO),
     -  texo(#MO),tesi(#MO)
      common /grid/ ncover(#GX,#GX,#GX),igfree(#CV),ngfree,ngrid,
     -  ng1,ng2,ng3,fltngr,fltng3,grid,e2g,e2gm,rsppe2,rspme2
      character*80 label
...
104       continue
10    continue
      if (ngfree .le. #CV) return
      write (6,1000) ngfree
1000  format(' Number of free gridpoints=',i8,' exceeds the capacity ',
     -  'of the program (#CV)',/,' - recompile with increased # CV')
      stop
      end
...
c*****Save all the common blocks
      ...
      common /svint/ n3(29)
      common /info/ n4(8)
      common /grid/ n5(#GX,#GX,#GX),n6(#CV),n7(5),x4(7)
      common /coord/ x5(7,#SV),n8(#SV)
      common /dploy/ x6(3,#SV),x7(15)
      common /ctotal/ x8(12,#NA),x9(24,#MO)
      common /energy/ d10(4,#MO),x11(8,#MO),d12(4),x13(4,#IJ)
      common /cavdat/ x14(6)
      common /cavprb/ n9(2,#MO),x17(4)
      common /ecell/ x15(81)
      common /option/ n10(40)
      common /spcpar/ x16(9)
      common /genslv/ x19(4,#SV),x20(3,#SV,#SV)
      common /cntrl/ x21(4,100)
      common /distr/ d22(2,#RG),n11(22,#RG),x24(20,#RG),x25(31),n12(6)
      rewind iunit
      read (iunit)
...
c      if (#CV .ge. 2**15-1) then
c        write (6,130)
c        stop
c      end if
      if (nslv .gt. #SV .or. ngrid .gt. #GX .or.
     -  nmolg .gt. #MO .or. nmolr .gt. #MO .or.
     -  nmolg*(nmolg-1)/2 .gt. #IJ .or. nmolg*nslv .gt. #NA .or.
     -  nmolr*(nmolr-1)/2 .gt. #IJ .or. nmolr*nslv .gt. #NA) then
        write (6,131)
        stop
      end if
999   return
...
c*****Compute the total energy esm of a configuration from scratch
      ...
      dimension tesi(#MO),tex(#MO),ex(#IJ),c(3,#NA),crm(3,#MO),
     -  nr(#RG),angcr(#RG),or(3,3,#MO)
      common /texz/ tex0(#MO)
      common /nmbrs/ autokc,autoa,rdtodg,boltz,pi,pi2
      dimension rrj(#MO),ori(3)
      esm=0.d0
      do i=1,nmolec
        tesi(i)=0.d0
      end do
...
c*****old value: esmo, new value:esmn.
      ...
      dimension tex(#MO),tesi(#MO),c(3,#NA),crm(3,#MO)
      common /nmbrs/ autokc,autoa,rdtodg,boltz,pi,pi2
      esmnew=0.d0
      tex(is)=0.0
      i1=is-1
...
c*****Translate and rotate the isolv-th molecule
     ...
     -  prsag,prsar,pressg,pressr,bparg,bparr
      ...
      common /info/ nslv,nmolg,nmolr,natg,natr,icopvg,icopvr,nmolmx
      common /dploy/ rsi(3,#SV),crmi(3),orj(3,3),cedgs,rotax,delvol
      dimension rno(5),rsj(3,#SV),ch(3),crmfac(3)
      call random(5,rno)
...
c*****Rotate a molecule by phi around iaxis
      common /coord/ relcrd(3,#SV),cslv(3,#SV),qslv(#SV),islv(#SV)
      dimension rmass(3),rot(3,3),ori(3,3),orj(3,3),csb(3,#SV)
      csp=cos(phi)
      snp=sin(phi)
      i1=mod(iaxis,3)+1
      i2=mod(iaxis+1,3)+1
...
c     crm is in current volume scale
      dimension c(3,#NA),crm(3),orient(3,3,#MO)
      common /info/ nslv,nmolg,nmolr,natg,natr,icopvg,icopvr,nmolmx
      common /coord/ relcrd(3,#SV),cslv(3,#SV),qslv(#SV),islv(#SV)
      do 20 i=1,nslv
        do 20 j=1,3
...
c*****Create new molecules from nmol1+1 to nmol2
      dimension c(3,#NA),crm(3,#MO),or(3,3,#MO),crmi(3)
...
c*****Obtain the rotation matrices for each molecule from c
      character*8 it
      dimension  c(3,#NA),crm(3,#MO),orient(3,3,#MO),relcrd(3,nslv),
     -    islv(nslv),crmi(3),idel(#MO)
...
c*****Compute center of mass of a molecule
      dimension csa(3,#SV),rmass(3),ian(#SV)
      common /atwght/ avgd,aw(100)
      fnumx=0.
      fnumy=0.
      fnumz=0.
...
1015  format(/,' Distribution functions were based on ',i6,
     ...
     -  ' configurations in the ',a8)
      ...
      dimension cumsum(10,#RG),sd(#RG),itype(3),ncnfb(10),blav(10)
      character*12 itype
      character*8 isyst
